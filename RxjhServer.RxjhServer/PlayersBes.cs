...
namespace RxjhServer.RxjhServer
{
	public class PlayersBes : 气功属性
	{
		...
		public ThreadSafeDictionary<double, GroundItems> 地面物品列表;
		...
		public List<MartialArts> SkillCombo;

		public int SkillCombo_Index_Start = 0;

		public int SkillCombo_Index_Stop = 0;

		public MartialArts[,] Array_Skill_Book = new MartialArts[4, 35];

		public CoordinateClass 新坐标;
		...
		public PlayersBes(NetState client)
		{
			...
			地面物品列表 = new ThreadSafeDictionary<double, GroundItems>();
			...
			土灵符坐标 = new Hashtable();
			攻击列表 = new List<攻击类>();
			SkillCombo = new List<MartialArts>();
			传书列表 = new Dictionary<int, EmailClass>();
			Array_Skill_Book = new MartialArts[4, 35];
		...
		public byte[] Get土灵符byte()
		{
			...
			try
			{
				foreach (DictionaryEntry item in 土灵符坐标)
				{
					...
					if (num >= 10)
					{
						Buffer.BlockCopy(Encoding.GetEncoding(1252).GetBytes(CoordinateClass.Rxjh_Name), 0, array, (num - 10) * 32, Encoding.GetEncoding(1252).GetBytes(CoordinateClass.Rxjh_Name).Length);
						Buffer.BlockCopy(BitConverter.GetBytes(CoordinateClass.Rxjh_Map), 0, array, (num - 10) * 32 + 15, 4);
						Buffer.BlockCopy(BitConverter.GetBytes(CoordinateClass.Rxjh_X), 0, array, (num - 10) * 32 + 19, 4);
						Buffer.BlockCopy(BitConverter.GetBytes(CoordinateClass.Rxjh_Y), 0, array, (num - 10) * 32 + 23, 4);
						Buffer.BlockCopy(BitConverter.GetBytes(CoordinateClass.Rxjh_Z), 0, array, (num - 10) * 32 + 27, 4);
					}
				}
			}
			catch (Exception ex)
		...
		public void 地面物品增加(Dictionary<long, GroundItems> Itmes)
		{
			try
			{
				if (Itmes != null && Itmes.Count > 0)
				{
					using (PacketData packetData = new PacketData())
					{
						...
						foreach (GroundItems value in Itmes.Values)
						{
		...
		public void 读出人物数据()
		{
			try
			{
				...
				新坐标 = new CoordinateClass(Player_FLD_X, Player_FLD_Y, Player_FLD_Z, Player_FLD_Map);
					...
					if (num6 < 78)
					{
						...
						try
						{
							if (array7.Length >= num6 * 6 + 4)
							{
								...
								if (num7 != 0)
								{
									MartialArts MartialArts = new MartialArts(num7);
									if ((MartialArts.FLD_JOB == 0 || Player_Job == MartialArts.FLD_JOB) && (MartialArts.FLD_ZX == 0 || Player_Zx == MartialArts.FLD_ZX) && Player_Job_Level >= MartialArts.FLD_JOBLEVEL && Player_Level >= MartialArts.FLD_LEVEL)
									{
										Array_Skill_Book[MartialArts.FLD_武功类型, MartialArts.FLD_INDEX] = MartialArts;
										if (num8 != 0)
										{
											if (num8 > MartialArts.最高武功_等级)
											{
												num8 = MartialArts.最高武功_等级;
											}
											MartialArts.武功_等级 = num8;
										}
									}
								}
								goto IL_1698;
				...
				if (dBToDataTable.Rows[0]["FLD_升天武功"] != DBNull.Value)
				{
					...
					for (int m = 0; m < 18 && array10.Length >= m * 8 + 8; m++)
					{
						...
						MartialArts MartialArts2 = new MartialArts(fLD_PID_);
						MartialArts2.武功_等级 = 武功_等级;
						if (MartialArts2.武功_等级 > MartialArts2.最高武功_等级)
						{
							MartialArts2.武功_等级 = MartialArts2.最高武功_等级;
						}
						if ((MartialArts2.FLD_JOB == 0 || Player_Job == MartialArts2.FLD_JOB) && (MartialArts2.FLD_ZX == 0 || Player_Zx == MartialArts2.FLD_ZX) && Player_Job_Level >= MartialArts2.FLD_JOBLEVEL && Player_Level >= MartialArts2.FLD_LEVEL)
						{
							Array_Skill_Book[MartialArts2.FLD_武功类型, MartialArts2.FLD_INDEX] = MartialArts2;
						}
					}
				}
				...
				if (array31.Length >= 128)
				{
					for (int num16 = 0; num16 < 30; num16++)
					{
						try
						{
							...
							{
								CoordinateClass CoordinateClass = new CoordinateClass(BitConverter.ToSingle(array32, 0), BitConverter.ToSingle(array33, 0), BitConverter.ToSingle(array34, 0), BitConverter.ToInt32(array35, 0));
								CoordinateClass.Rxjh_Name = @string;
								if (土灵符坐标.ContainsKey(10 + num16))
								{
									土灵符坐标.Remove(10 + num16);
								}
								土灵符坐标.Add(10 + num16, CoordinateClass);
							}
						}
						catch (Exception)
						{
		...
		public double[] getRateCuongHoa(double MinRate, double MaxRate, HcItimesClass BuaMayMan = null)
		{
			...
			if (FLD_VIP == 1)
			{
				array[0] += 100.0 * World.VipSynthesisRate;
				array[1] += 100.0 * World.VipSynthesisRate;
			}
			if (World.合成率 != 0.0)
			{
