...
namespace YulgangServer.RxjhServer
{
    public class 异常状态类 : IDisposable
    {
        ...
        public 异常状态类(Players play, int 时间, int 异常id, int 异常数量)
        {
            ...
            FldPid = 异常id;
            Time = DateTime.Now;
            Time = Time.AddMilliseconds((double) 时间 + 0);
            Play = play;
            Npcyd = new System.Timers.Timer((double)时间 + 0);
        ...
        public 异常状态类(NpcClass play, int npcPlayId, int 时间, int 异常id, int 异常数量)
        {
            ...
            FldPid = 异常id;
            Time = DateTime.Now;
            Time = Time.AddMilliseconds((double)时间 + 1000);
            Npc = play;
            Npcyd = new System.Timers.Timer((double)时间 + 1000);
            Npcyd.Elapsed += 时间结束事件1;
            Npcyd.Enabled = true;
            Npcyd.AutoReset = false;
        ...
        public void yczt_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            if (Play != null)
            {
                ...
                if ((Play.Player_Job == 2) || (Play.Player_Job == 9))
                {
                    ...
                    if ((num <= Play.剑_升天三气功_火凤临朝) && (Play.Player_FLD_HP <= 0L))
                    {
        ...
        public void EndEvent()
        {
            ...
            try
            {
                ...
                if (Npc != null)
                {
                    ...
                    {
                        case 1:
                            //this.Npc.FldAt += this.Npc.FLD_TRUDAME_NPC_CAMSU;
                            Npc.FLD_TRUDAME_NPC_CAMSU = 0;
                            break;

                        case 2:
                            //this.Npc.FldDf += this.Npc.FLD_TRUDEF_NPC_CAMSU;
                        ...
                        //only JOB 6
                        case 9:
                            //this.Npc.FldDf += this.Npc.FLD_TRUDEF_NPC_NINJA;
                ...
                else if (Play != null)
                {
                    ...
                    {
                        case 1:
                            //this.Play.FLD_TRUDAME_CAMSU = 0;
                            Play.UpdatePowersAndStatus();
                            break;

                        case 2:
                            //this.Play.FLD_TRUDEF_CAMSU = 0;
                            Play.UpdatePowersAndStatus();
                            break;
                        //only JOB 7
                        case 22:
                            Play.FLD_TRUDAME_CAMSU = 0;
                            Play.UpdatePowersAndStatus();
                            break;

                        //only JOB 7
                        case 25:
                            Play.FLD_TRUDEF_CAMSU = 0;
                            Play.UpdatePowersAndStatus();
                            break;

                        //only JOB 7: Skill Buff 140
                        case 20:
                            Play.FLD_TANGDEF_CAMSU = 0;
                            Play.UpdatePowersAndStatus();
                            break;

                        //only JOB 6
                        case 9:
                            Play.FLD_TRUDEF_NINJA = 0;
                            Play.UpdatePowersAndStatus();
                            break;
            ...
            {
                Form1.WriteLine(1, string.Concat(new object[] { "异常状态类 时间结束事件 出错：[", FldPid, "]", exception }));
            }
            finally
            {
                Dispose();
        ...
        public void 状态效果(int 异常id, int 开关, int 异常数量, int 时间)
        {
            ...
            Buffer.BlockCopy(BitConverter.GetBytes(异常id), 0, dst, 19, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(异常id), 0, dst, 59, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(开关), 0, dst, 23, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(开关), 0, dst, 63, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(时间), 0, dst, 39, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(异常数量), 0, dst, 43, 4);
            if (Play != null)
            {
                Buffer.BlockCopy(BitConverter.GetBytes(Play.UserSessionID), 0, dst, 15, 2);
                Buffer.BlockCopy(BitConverter.GetBytes(Play.UserSessionID), 0, dst, 5, 2);
                if (Play.Client != null)
                {
                    Play.Client.Send多包(dst, dst.Length);
                }
                Play.发送当前范围广播数据多包(dst, dst.Length);
            }
            else if (Npc != null)
            {
                Buffer.BlockCopy(BitConverter.GetBytes(Npc.FldIndex), 0, dst, 15, 2);
                Buffer.BlockCopy(BitConverter.GetBytes(Npc.FldIndex), 0, dst, 5, 2);
                Npc.广播数据(dst, dst.Length);
            }
        }
